...
def get_database_path(shop):
    ...
    logger.debug(f"[get_databases_path] Path for shop '{shop}': {db_path}")
    return db_path
...
def create_tables(shop):
    logger.debug(f"[create_tables] Creating tables for shop: {shop}")
    try:
        db_path = get_database_path(shop)
        ...
        connection.commit()
        logger.info(f"[create_tables] Tables created successfully for shop: {shop}")
    except sqlite3.Error as e:
        logger.error(f"[create_tables] Error creating tables for shop {shop}: {e}")
    finally:
        connection.close()
        logger.debug(f"[create_tables] Connection closed for shop: {shop}")

# Sauvegarder les produits
def save_products_to_db(shop, products):
    logger.debug(f"[save_products_to_db] Saving products for shop: {shop}")
    ...
    try:
        ...
        connection.commit()
        logger.info(f"[save_products_to_db] Products saved successfully for shop: {shop}")
    except sqlite3.Error as e:
        logger.error(f"[save_products_to_db] Error saving products for shop {shop}: {e}")
    finally:
        connection.close()
        logger.debug(f"[save_products_to_db] Connection closed for shop: {shop}")
    
# Sauvegarder les commandes
def save_orders_to_db(shop, orders):
    logger.debug(f"[save_orders_to_db] Saving orders for shop: {shop}")
    ...
    try:
        ...
        connection.commit()
        logger.info(f"[save_orders_to_db] Orders saved successfully for shop: {shop}")
    except sqlite3.Error as e:
        logger.error(f"[save_orders_to_db] Error saving orders for shop {shop}: {e}")
    finally:
        connection.close()
        logger.debug(f"[save_orders_to_db] Connection closed for shop: {shop}")

# Sauvegarder les clients
def save_customers_to_db(shop, customers):
    logger.debug(f"[save_customers_to_db] Saving customers for shop: {shop}")
    create_tables(shop)
    db_path = get_database_path(shop)
    try:    
        connection = sqlite3.connect(db_path)
        ...
        connection.commit()
        logger.info(f"[save_customers_to_db] Customers saved successfully for shop: {shop}")
    except sqlite3.Error as e:
        logger.error(f"[save_customers_to_db] Error saving customers for shop {shop}: {e}")
    finally:
        connection.close()
        logger.debug(f"[save_customers_to_db] Connection closed for shop: {shop}")

# Sauvegarder les articles de commande
def save_order_items_to_db(shop, order_items):
    logger.debug(f"[save_order_items_to_db] Saving order items for shop: {shop}")
    create_tables(shop)
    db_path = get_database_path(shop)
    try:
        connection = sqlite3.connect(db_path)
        ...
        connection.commit()
        logger.info(f"[save_order_items_to_db] Order items saved successfully for shop: {shop}")
    except sqlite3.Error as e:
        logger.error(f"[save_order_items_to_db] Error saving order items for shop {shop}: {e}")
    finally:
        connection.close()
        logger.debug(f"[save_order_items_to_db] Connection closed for shop: {shop}")