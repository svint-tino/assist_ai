...
export function InputBox({
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch(
        "http://localhost:4000/api/assistant/conversation",
        {
          method: "POST",
      ...
      const errorMessage: Message = {
        role: "assistant",
        content: "Erreur de réponse de KOOC.",
        type: "text",
      };
  ...
  const renderMessage = (msg: Message, index: number) => {
    const baseClasses = `max-w-[85%] sm:max-w-[80%] px-3 sm:px-4 py-2 rounded-lg text-sm ${
      msg.role === "user"
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full max-w-4xl mx-auto">
      {/* Zone des messages */}
      <div className="flex flex-col-reverse gap-4 max-h-[60vh] sm:max-h-[70vh] overflow-y-auto w-full px-4">
      ...
      {/* Zone de saisie */}
      <div className="flex gap-2 items-end w-full max-w-2xl px-4">
        <textarea
          ref={textareaRef}
          value={question}
          onChange={(e) => {
          ...
          rows={1}
          className="flex-1 border border-gray-300 rounded-lg px-3 sm:px-4 py-2 text-sm sm:text-base resize-none max-h-[120px] overflow-y-auto focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent"
        />

        <button
          onClick={handleSend}
          className="bg-black text-white px-4 sm:px-6 py-2 rounded-lg hover:bg-gray-800 disabled:opacity-50"
          disabled={isLoading}
        >
          Send
        </button>
